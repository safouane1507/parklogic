...
GeneratedMap WorldGenerator::generate(const MapConfig& config) {
    ...
    auto createFacility = [&](int type, int size, bool isTop) -> std::unique_ptr<Module> {
        if (type == 0) { 
            if (size == 0) return std::make_unique<SmallParking>(isTop);
            else return std::make_unique<LargeParking>(isTop);
        } else {
            if (size == 0) return std::make_unique<SmallChargingStation>(isTop);
            else return std::make_unique<LargeChargingStation>(isTop);
        }
    };

    auto getNextFacility = [&](bool isTop) -> std::unique_ptr<Module> {
        std::vector<int> available;
        if (smallParkingLeft > 0) available.push_back(0);
        if (largeParkingLeft > 0) available.push_back(1);
        if (smallChargingLeft > 0) available.push_back(2);
        if (largeChargingLeft > 0) available.push_back(3);
    ...
    while (true) {
        ...
        PlannedUnit unit;
        int rType = (totalLeft >= 2) ? std::uniform_int_distribution<>(0, 2)(gen) : std::uniform_int_distribution<>(0, 1)(gen);
        if (rType == 0) { unit.road = std::make_unique<UpEntranceRoad>(); unit.topFacility = getNextFacility(true); }
        else if (rType == 1) { unit.road = std::make_unique<DownEntranceRoad>(); unit.bottomFacility = getNextFacility(false); }
        else { unit.road = std::make_unique<DoubleEntranceRoad>(); unit.topFacility = getNextFacility(true); unit.bottomFacility = getNextFacility(false); }
        if (unit.topFacility) unit.topFacility->setParent(unit.road.get());
        if (unit.bottomFacility) unit.bottomFacility->setParent(unit.road.get());
    ...
    for (auto &unit : plan) {
        ...
        while (collision) {
            ...
            if (getFacLeftX(unit.topFacility, {0, -1}) < safeX_top || 
                getFacLeftX(unit.bottomFacility, {0, 1}) < safeX_bottom || 
                roadWorldPos.x < safeX_road) {
                placeRoadAt(currentX, startY);
                safeX_road = currentX; 
                collision = true;
            }
    ...
    for (const auto& mod : modules) facMaxX = std::max(facMaxX, mod->worldPosition.x + mod->getWidth());
    
    // Ensure road covers all facilities
    while (currentX < (facMaxX - 0.1f)) placeRoadAt(currentX, startY);
